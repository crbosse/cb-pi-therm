...
module.exports = function(homebridge) {
  ...
  homebridge.registerAccessory('homebridge-cb-therm', 'Therm', Thermostat);
};
...
class Thermostat {
  constructor(log, config) {
    ...
    //add cb to everything in order to distinguish it from the OG
    this.fanRelayPin_cb = config.fanRelayPin_cb || 26;
    this.heatRelayPin_cb = config.heatRelayPin_cb || 18;
    this.coolRelayPin_cb = config.coolRelayPin_cb || 20;
    this.dhtSensorType = config.dhtSensorType || 22;
    this.temperatureSensorPin_cb = config.temperatureSensorPin_cb || 4;
    ...
    HeatingCoolingStateToRelayPin = {
      [Characteristic.CurrentHeatingCoolingState.HEAT]: this.heatRelayPin_cb,
      [Characteristic.CurrentHeatingCoolingState.COOL]: this.coolRelayPin_cb
    };

    gpio.setup(this.fanRelayPin_cb, gpio.DIR_HIGH);
    gpio.setup(this.heatRelayPin_cb, gpio.DIR_HIGH);
    gpio.setup(this.coolRelayPin_cb, gpio.DIR_HIGH);
  ...
  readTemperatureFromSensor() {
    dhtSensor.read(this.dhtSensorType, this.temperatureSensorPin_cb, (err, temperature, humidity) => {
      if (!err) {
